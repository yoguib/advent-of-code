...
func main() {
	...
	for _, idRange := range parsedData {
		...
		go func(ids string) {
			...
			fmt.Println("invalid ids:", response)
...
func getInvalidIdsByRange(start, end int) []int {
	...
	for i := start; i <= end; i++ {
		...
		if length%2 == 0 {
			...
			// fmt.Println("Skip Amount", skipAmount, "data", asString, "length", length)
			previous := ""
			match := false
		...
		// hasPair := false
		// previousItem := ""
		// for i, item := range asString {
		// 	if i == 0 {
		// 		previousItem = string(item)
		// 		continue
		// 	}

		// 	parsedItem := string(item)

		// 	if parsedItem == previousItem {
		// 		hasPair = true
		// 	} else {
		// 		hasPair = false
		// 	}
